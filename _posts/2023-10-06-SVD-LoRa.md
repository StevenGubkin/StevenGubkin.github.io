...
Now let's look at $$(U_A^i)^\top U_B^j$$ which I will call $$S$$ for "subspace similarity".  Let's think about this column by column.  Let the $$k^{th} $$ column of $$U_A$$ and $$U_B$$ be denoted by $$\vec{u}_{Ak$}$$ and $$\vec{u}_{Bk}$$ respectively.  Then the $$k^{th}$$ column of $$(U_A^i)^\top U_B^j$$ would be 

$$
\begin{bmatrix}
\vec{u}_{A1} \cdot \vec{u}_{Bk}\\
\vec{u}_{A2} \cdot \vec{u}_{Bk}\\
\vdots\\
\vec{u}_{Ai} \cdot \vec{u}_{Bk}\\
\end{bmatrix}
$$

These are just the *coefficients* of the orthogonal projection of $$\vec{u}_{Bk}$$ onto the image of $$U_A^i$$.  The actual projection would be

$$
\textrm{Proj}_{\textrm{Im}(U_A^i)}(\vec{u}_{Bk}) = U_A^i (U_A^i)^\top \vec{u}_{Bk} = U_A^i (U_A^i)^\top U_B^j \vec{e}_k
$$

So $$(U_A^i)^\top U_B^j$$ is a very natural thing to look at:  it records the coefficients of the orthogonal projections of the column vectors of $$U_B^j$$ onto the column space of $$U_A^i$$.

We are then taking the square of the [Frobenius norm](https://en.wikipedia.org/wiki/Matrix_norm#Frobenius_norm) of this matrix.  We really just need to know the definition:

$$
\vert A \vert_\text{F} = \sqrt{\sum_{i}^m\sum_{j}^n |a_{ij}|^2} = \sqrt{\operatorname{trace}\left(A^* A\right)} = \sqrt{\sum_{i=1}^{\min\{m, n\}} \sigma_i^2(A)}
$$
...
Let $$\sigma_k$$ be the singular values of $$S = (U_A^i)^\top U_B^j$$.  Then 
...
**Lemma 1** Let $$M$$ be a matrix with orthonormal columns.  Then for any vector $$\vec{v}$$ we have $$\vert M \vec{v}\vert = \vert\vec{v}\vert$$
...
\begin{align*}
\vert M \vec{v} \vert^2 
&= \langle  M \vec{v},  M\vec{v}\rangle\\
&= \langle   \vec{v},  M^\top M \vec{v}\rangle\\
&= \langle   \vec{v},  \mathbb{I} \vec{v}\rangle\\
&= \langle   \vec{v}, \vec{v}\rangle\\
&= \vert \vec{v} \vert^2
\end{align*}
$$

**Lemma 2** The singular values of $$(U_A^i)^\top U_B^j$$ are at most $$1$$. 

Proof:

Let $$\vec{\beta}$$ be a vector of length $$1$$ in $$\mathbb{R}^j$$.  By lemma 1, $$\vec{b} = U_B^j \vec{\beta}$$ is a unit vector in the image of $$U_B^j$$.  So $$(U_A^i)^\top U_B^j \vec{\beta} = (U_A^i)^\top \vec{b}$$.  By lemma 1 again, $$\vert (U_A^i)^\top \vec{b}\vert = \vert (U_A^i) (U_A^i)^\top \vec{b}\vert  $$.  However, $$\vert (U_A^i) (U_A^i)^\top \vec{b}\vert $$ is the length of the projection of $$\vec{b}$$ onto the image of $$U_A^i$$, which is clearly less  or equal to $$\vert \vec{b} \vert = 1$$ (with equality if and only if $$\vec{b} = U_B^j \vec{\beta}$$ is in the image of $$U_A^i$$).  Thus we have that $$\sigma_1 \leq 1$$, and so all of the singular values of $$(U_A^i)^\top U_B^j$$ are at most $$1$$. 
...
Let us examine two other special cases:

* Case 1:  $$j\leq i$$ and $$\textrm{Im}(U_B^j) \subset \textrm{Im}(U_A^i)$$
   * In this case we have $$\vert(U_A^i)^\top U_B^j \vec{\beta}\vert = 1$$ for all unit vectors $$\vec{\beta}$$.  The columns of $$U_B^j$$ are right singular vectors, and the columns of $$U_A^i$$ left singular vectors, for $$\vert(U_A^i)^\top U_B^j$$.  The singular values are all $$1$$.  We have

   $$
   \phi(A,B,i,j) = \frac{1}{j}\sum_{k=1}^{j} \sigma_k^2 = 1
   $$

* Case 2: $$i \leq j$$ and $$\textrm{Im}(U_A^i) \subset \textrm{Im}(U_B^j) $$.

  The same reasoning shows that the first $$i$$ singular values are all $$1$$, so

   $$
   \phi(A,B,i,j) = \frac{1}{i}\sum_{k=1}^{i} \sigma_k^2 = 1
   $$

So as long as one subspace contains the other the similarity measure is equal to $$1$$!
